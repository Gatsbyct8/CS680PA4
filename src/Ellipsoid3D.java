...
public class Ellipsoid3D
{
	...
	private void fillMesh()
	{
		...
		for (i = 0, phi = -pi/2; i < stacks; ++i, phi += dphi){
			...
			for (j = 0, theta = -pi; j < slices; ++j, theta += dtheta) {
				...
				// Compute vertex
				mesh.v[i][j].x = center.x + rx * (float)(cos_phi * cos_theta);
				mesh.v[i][j].y = center.y + ry * (float)(cos_phi * sin_theta);
	...
	public void draw(Quaternion viewing_quaternion, Point3D viewing_center, Point2D[] tri, Point3D view_vector, Point3D triangle_normal, BufferedImage buff, boolean doSmooth, InfiniteLight infiniteLight, Material mat_sphere, PointLight pointLight, AmbientLight ambientLight){
		...
		lightUtil.rotate(infiniteLight, pointLight, viewing_quaternion, viewing_center);
